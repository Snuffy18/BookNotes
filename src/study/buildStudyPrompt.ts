...
export function buildStudyPreferencesInstructions(p: StudyPreferencesSnapshot): string {
  ...
  if (p.highlightKeyElements) {
    ...
    if (bits.length > 0) {
      highlightLine = `Highlighting in output: In summary and detailedNotes, wrap ${bits.join(
        ", "
      )} with **double asterisks** (markdown bold) so they stand out. Only mark text that is clearly those categories on the page. If nothing qualifies, use plain text without markers.`;
    }
  }
