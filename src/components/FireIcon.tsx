...
/** Vector from `assets/fire.svg` (SVG Repo flame with gradient). */
const PATH_MAIN =
...
const VIEWBOX = "-33 0 255 255";

type Props = {
  size?: number;
};

export function FireIcon({ size = 16 }: Props) {
  const rawId = useId();
  const gradId = `firelg_${rawId.replace(/[^a-zA-Z0-9]/g, "_")}`;
  ...
  return (
    <Svg width={size} height={size} viewBox={VIEWBOX}>
      ...
      <G>
        <Path d={PATH_MAIN} fill={`url(#${gradId})`} fillRule="evenodd" />
        <Path d={PATH_MID} fill="#fc9502" fillRule="evenodd" />
        <Path d={PATH_TIP} fill="#fce202" fillRule="evenodd" />
      </G>
    </Svg>
  );
}
